...
honSort = filloutliers(honolulu,'linear');
...
%% convert to CSV
writetable(array2table(honSort),'honolulu.csv');
...
%plot(avgLocal(:,1),avgLocal(:,2),'LineWidth',2); %average 
xlabel('Years');
ylabel('Sea Level (mm)')
legend('honolulu','nawiliwili','kahului','hilo','mokuoloe','kawaihae');

%% make an array of years by local stations(after outliers removed)
...
avgLocal = NaN*zeros(length(totalYears),2);
avgLocal(:,1) = totalYears;
avgLocal(:,2) = nanmean(seaLevel_grid(:,2:7),2);
...
%% Step #2c: Calculate difference from start year at each station
...
for num = 2:7
    plot(seaChange_grid(:,1),seaChange_grid(:,num), 'LineWidth',2);
    hold on
end
...
ylabel('Sea Level (mm)')
legend('honolulu','nawiliwili','kahului','hilo','mokuoloe','kawaihae');
%% Plotting for difference from starting year
for i = 1:6
